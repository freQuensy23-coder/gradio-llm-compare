...
def llm1(history: DialogHistory) -> str:
    ...
    gen_ids = model1.generate(**input_ids.to(model1.device), temperature=1, max_new_tokens=64)[0][len(input_ids):]
    gen_text = tokenizer1.decode(gen_ids)
    return gen_text
...
def llm2(history: DialogHistory) -> str:
    ...
    gen_ids = model2.generate(**input_ids.to(model2.device), temperature=1, max_new_tokens=64)[0][len(input_ids):]
